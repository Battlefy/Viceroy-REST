...
ViceroyRest.prototype._request = function(method, url, query, callback) {
  ...
  if(query._id) {
    ...
    if(!query._id.$in) { query._id = { $in: [query._id] }; }
    ...
    if(!request) {
      ...
      for(var i = 0; i < query._id.$in.length; i += 1) {
        if(request.query._id.$in.indexOf(query._id.$in[i]) == -1) {
          request.query._id.$in.push(query._id.$in[i]);
        }
      }
