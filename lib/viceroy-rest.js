...
function ViceroyRest(opts) {
  ...
  if(typeof opts.host != 'string') { throw new Error('opts.host must be a string'); }
  if(typeof opts.port != 'number' && typeof opts.port != 'string') { throw new Error('opts.number must be a number'); }
  if(opts.beforeSend && typeof opts.beforeSend != 'function') { throw new Error('opts.beforeSend must be a function'); }
...
ViceroyRest.prototype.connect = function(cb) {
  ...
  superagent.Request.prototype.withCredentials = function() {
    if(_this.opts.withCredentials === true) {
...
ViceroyRest.prototype.find = function(query, opts, cb) {
  ...
  if(query._id) {
    ...
    if(typeof queryIds != 'object') { queryIds = [queryIds]; }
    if(queryIds.$in) { queryIds = queryIds.$in; }
    delete query._id;
...
ViceroyRest.prototype.insert = function(records, opts, cb) {
  ...
  // validate
  if(typeof cb != 'function') { throw new Error('cb must be an object'); }
  if(records === null || typeof records != 'object' || typeof records.length != 'number') { 
    return cb(new Error('records must be an array'));
  }
  if(opts === null || typeof opts != 'object') { return cb(new Error('opts must be an object')); }

  // call before send
  if(this.opts.beforeSend) {
    var _records = this.opts.beforeSend(records);
 ...
 * Update
 * @param  {Object}   query   Viceroy query object.
 * @param  {Object}   delta   Viceroy delta object.
 * @param  {Object}   opts    Update opts.
 * @param  {Function} cb      Executed upon completion.
 */
...
ViceroyRest.prototype._startScheduler = function() {
  ...
  process.nextTick(function() {
    ...
    for(var key in _this._requests) {
      if(_this._requests.hasOwnProperty(key)) {
        (function(request) {
          ...
          r.end(function(err, res) {
            ...
            if(typeof res.body != 'object' || typeof res.body.length != 'number') {
              return cb(new Error('server response from ' + request.url + ' does not contain the expected JSON array of records'));
            }
